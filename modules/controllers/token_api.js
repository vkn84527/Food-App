...
module.exports.check_bookings = async function (req, res) {
    try {
        var sql_query = 'select * from token_order where order_id=? ';
        var values = [req.body.order_id]
        ...
        if (results.length == 0) {
            ...
            responses.sendordervalueResponse(res, 'Your order_id is valid', results, constants.STATUS_CODES.SUCCESS)
        }
    }
    catch {
...
module.exports.token = async function (req, res) {
    ...
    try {
        ...
        var sql_query01 = 'select product_id from  product where product_id=?'
        var values01 = [req.body.product_id]
        var results01 = await execute_query(sql_query01, values01)

        if (results01.length == 0) {
            responses.sendResponse(res, "Your product_id is Not Valid !!! ", constants.STATUS_CODES.BAD_REQUEST)
            ...
            // }
            // else {
                var sql_query1 = 'SELECT b.customer_id,b.merchant_id,b.ammount from booking b join product p on b.merchant_id=p.merchant_id where p.product_id=?'
                var values1 = [req.body.product_id]
                var results1 = await execute_query(sql_query1, values1)
                console.log(results1)
                //console.log(results1[0].customer_id,results1[0].merchant_id,results1[0].ammount)

                var sql_query = 'insert into token_order(product_id,auto_assignment,job_description,job_pickup_phone,job_pickup_name,job_pickup_email,job_pickup_address\
,job_pickup_datetime,customer_email,customer_username,customer_phone,customer_address,product_name,job_delivery_datetime,customer_id,merchant_id,ammount) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
                const values = [
                    req.body.product_id,
                ...
                let results = await execute_query(sql_query, values)
                //console.log(results)
                var sql_query11 = 'SELECT order_id,customer_id,merchant_id,ammount,job_delivery_datetime from  token_order where order_id=?'
                var values11 = [results.insertId]
                var results11 = await execute_query(sql_query11, values11)
                //console.log(results11)
...
// var sql_query = 'insert into token_order(product_id,auto_assignment,job_description,job_pickup_phone,job_pickup_name,job_pickup_email,\
//     job_pickup_address\
//     ,job_pickup_datetime,customer_email,customer_username,customer_phone,customer_address,job_delivery_datetime,customer_id,\
//     merchant_id,ammount) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
//                     const values = [
//                         req.body.product_id,
//                         req.body.auto_assignment,
//                         req.body.job_description,
